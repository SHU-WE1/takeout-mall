<template>
  <div class="navbar">
    <div class="statusBox">
      ...
      <span v-else
            class="businessBtn closing">閉店中</span>
    </div>
    ...
    <div :key="restKey"
      ...
      <div class="rightStatus">
        ...
        <span class="navicon operatingState" @click="handleStatus"><i />営業状態設定</span>
      </div>
      <div class="avatar-wrapper">
        <div :class="shopShow?'userInfo':''"
          ...
          <el-button type="primary"
            ...
            {{ translateUserName(name) }}<i class="el-icon-arrow-down" />
          </el-button>
          <div v-if="shopShow"
               class="userList">
    ...
    <el-dialog title="営業状態設定"
      ...
      <el-radio-group v-model="setStatus">
        <el-radio :label="1">
          営業中
          <span>現在レストランは営業状態で、自動的にすべての注文を受け付けます。閉店をクリックして店舗の閉店状態に入ることができます。</span>
        </el-radio>
        <el-radio :label="0">
          閉店中
          <span>現在レストランは閉店状態で、営業時間内の予約注文のみを受け付けます。営業中をクリックして手動で営業状態を復元できます。</span>
        </el-radio>
      </el-radio-group>
      <span slot="footer"
            class="dialog-footer">
...
export default class extends Vue {
  ...
  webSocket() {
    ...
    if (typeof WebSocket == 'undefined') {
      ...
      this.websocket.onerror = function () {
        that.$notify({
          title: 'エラー',
          message: 'サーバーエラー、リアルタイムアラーム情報を受信できません',
          type: 'error',
          duration: 0,
        })
      }
  ...
  // 翻译用户名
  private translateUserName(userName: string) {
