...
export default class extends Vue {
  ...
  public init() {
    ...
    // 初始化已选择的菜品缓存
    this.checkedListAll.forEach((item: any) => {
      if (!this.ids.has(item.id)) {
        this.allDishList.push(item)
  ...
  private checkedListHandle(value: [string]) {
    ...
    // value 是一个只包含菜品名的数组 需要从 dishList和allDishList中筛选出 对应的详情
    // 操作添加菜品 - 同时从当前分类列表和全部列表中查找
    const list = [...this.dishList, ...this.allDishList].filter((item: any) => {
