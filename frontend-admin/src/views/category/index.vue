      ...
      <el-table v-if="tableData.length"
        ...
        <el-table-column prop="type"
          ...
          <template slot-scope="scope">
            <span>{{ scope.row.type == '1' ? '料理分類' : 'セットメニュー分類' }}</span>
          </template>
        </el-table-column>
...
export default class extends Vue {
      ...
      name: [
        {
          ...
          validator: (rule: any, value: string, callback: Function) => {
            // const reg = /[\u4e00-\u9fa5]/
            var reg = new RegExp('^[A-Za-z\u4e00-\u9fa5\u3040-\u309F\u30A0-\u30FF]+$')
            if (!value) {
      ...
      sort: [
        {
          ...
          validator: (rule: any, value: string, callback: Function) => {
            if (value || String(value) === '0') {
              const reg = /^\d+$/
              if (!reg.test(value)) {
                callback(new Error('並び順は数字のみ入力可能です'))
              } else if (Number(value) > 99) {
                callback(new Error('並び順は0-99の数字のみ入力可能です'))
              } else {
                callback()
              }
            } else {
              callback(new Error('並び順は空にできません'))
            }
          }
        }
      ]
  ...
  private addClass(st: any) {
    if (st == 'class') {
      ...
      this.classData.title = '新規セットメニュー分類'
      this.type = '2'
    }
    this.action = 'add'
    this.classData.name = ''
  ...
  private statusHandle(row: any) {
    ...
    this.$confirm('この分類の状態を調整してもよろしいですか？', '確認', {
      ...
      enableOrDisableEmployee({ id: this.id, status: !this.status ? 1 : 0 })
        .then(res => {
          if (String(res.status) === '200') {
            this.$message.success('分類状態が正常に変更されました！')
            this.init()
          }
        })
        .catch(err => {
  ...
  private deleteHandle(id: any) {
    this.$confirm('この操作は分類を永久に削除します。続行しますか？', '削除確認', {
